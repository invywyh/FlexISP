...
def parse_kernel(name, blob):
  ...
  kernel.ppout = kConfig['RPixelPart'] # HACK for working with just the reduction; remove me
  ...
  for instruction in kReduce:
    ...
    # Operation
      ...
      if len(tIndices) == 1 and opcode in ['sum', 'min', 'max']:
        ...
        op = Op(opcode)
        op.result = (tName, tIndices[0]) # Package the name and index
      ...
      # If this is a regular operation
      elif opcode in ['mv', 'add', 'sub', 'mult', 'div', 'inv', 'mux', 'ne', 'eq', 'gt', 'lt', 'clamp', 'lshift', 'rshift', 'gte', 'lte', 'and', 'not', 'abs', 'or']:
        ...
        raise NotImplementedError("Encountered unknown opcode '%s'" % opcode)
